...
## Возможности (MVP)
<<<<<<< HEAD
- Ведение трат по типам: повседневные, крупные покупки, квартира (через кнопки).
- Справочники категорий/подкатегорий, импорт из Excel-шаблона `Копия Costs.xlsx` и дефолтные категории.
- Дашборд со средним расходом, лимитом и статусом “ТРАНЖИРА/В НОРМЕ”.
- Отчёты по периодам, экспорт в XLSX, импорт из XLSX (три листа).
- Ограничение доступа: владелец + белый список пользователей (по username).
=======
...
<<<<<<< HEAD
## Управление
- Кнопка **➕ Добавить расход** — пошаговое создание операции (тип → дата → детали → сумма → комментарий).
- **📊 Дашборд** — ключевые метрики и статусы.
- **📈 Отчёты** — быстрые периоды или произвольный диапазон.
- **📥 Импорт** — загрузка XLSX из шаблона.
- **📤 Экспорт** — выгрузка актуальных данных в XLSX совместимый с таблицей.

Доступ только у пользователей с username из `OWNER_USERNAME` или `ALLOWED_USERNAMES`.
